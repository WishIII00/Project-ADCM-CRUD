...
export default function SolarCellProducts({ onProductDelete }) {
  const [count, setCount] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  const fetchSolarCellProducts = () => {
    setLoading(true);
    axios.get("http://localhost:3000/api/newProducts/filter?category=Solar Cell")
